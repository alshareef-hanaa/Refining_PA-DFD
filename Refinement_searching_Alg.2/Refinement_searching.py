...
# TODO: check that every nodes/edges has type and label
...
# ?: {is this function to check if all edges have source and target ?}Hanaa
...
class Graph:
    ...
    # For comparisons
    # ?: {not sure what we compare here ?}Hanaa
    ...
    def ext(p, unmapped):
        if unmapped:  # is 'unmapped' non-empty?
            ...
            for c in p.cod.edges:
                ...
                #
                #
                # NOTE. In the recursive call to 'ext', we make a copy
                # of 'unmapped'.  This is necessary because 'unmapped'
                # is a mutable list and hence calls to 'ext' for
                # different "branches" (i.e. for different values of
                # 'c') would see the modified versions of 'unmapped'
                # from other branches if we didn't pass them a fresh
                # copy.

                n_ext = dict()
                if c_label == e_label:  # the labels are matched
                    if c_type == e_type or check_type(e_type, c_type):
                        if e_src in p.node_map:  # source of 'e' already mapped
                            if p.node_map[e_src] != c_src: continue
                        else:  # source of 'e' unmapped
                            if e_src_type == 'external_entity' and c_src_type == 'external_entity':
                                if e_src_label == c_src_label:
                                    n_ext[e_src] = c_src
                                else:
                                    continue
                            elif (e_src_type == 'process' and c_src_type == 'process') or (
                                    e_src_type == 'data_base' and c_src_type == 'data_base') or (
                                    e_src_type == 'composite_process' and c_src_type == 'composite_process'):
                                if e_src_label == c_src_label:
                                    n_ext[e_src] = c_src
                                else:
                                    continue
                            elif (
                                    e_src_type == 'process' or e_src_type == 'data_base' or e_src_type == 'composite_process') and (
                                    c_src_type == 'composite_process'):
                                if not any((p.cod.elements_labels[item] == e_src_label) and (
                                        p.cod.elements_types[item] == e_src_type) for item in p.cod.nodes):
                                    n_ext[e_src] = c_src
                                else:
                                    continue
                        if e_tgt in p.node_map:  # target of 'e' already mapped
                            if p.node_map[e_tgt] != c_tgt: continue
                        else:  # target of 'e' unmapped
                            if e_tgt_type == 'external_entity' and c_tgt_type == 'external_entity':
                                if e_tgt_label == c_tgt_label:
                                    n_ext[e_tgt] = c_tgt
                                else:
                                    continue
                            elif (e_tgt_type == 'process' and c_tgt_type == 'process') or (
                                    e_tgt_type == 'data_base' and c_tgt_type == 'data_base') or (
                                    e_tgt_type == 'composite_process' and c_tgt_type == 'composite_process'):
                                if e_tgt_label == c_tgt_label:
                                    n_ext[e_tgt] = c_tgt
                                else:
                                    continue
                            elif (
                                    e_tgt_type == 'process' or e_tgt_type == 'data_base' or e_tgt_type == 'composite_process') and (
                                    c_tgt_type == 'composite_process'):
                                if not any((p.cod.elements_labels[item] == e_tgt_label) and (
                    ...
                    continue
            if c_count == len(p.cod.edges):  # could not find any candidate c for e
                n_ext = dict()
                if (e_src in p.node_map) and (e_tgt not in p.node_map):  # source of 'e' already mapped
                    c_src = p.node_map[e_src]
                    if p.cod.elements_types[c_src] == 'composite_process':
                        n_ext[e_tgt] = c_src
                        p_ext = p.extended(n_ext, {e: 'O'})
                        result = ext(p_ext, rest)
                        results += result
                        return results
                elif (e_src not in p.node_map) and (e_tgt in p.node_map):  # target of 'e' already mapped
                    c_tgt = p.node_map[e_tgt]
                    if p.cod.elements_types[c_tgt] == 'composite_process':
                        n_ext[e_src] = c_tgt
                        p_ext = p.extended(n_ext, {e: 'O'})
                        result = ext(p_ext, rest)
                        results += result
                        return results
                elif (e_src in p.node_map) and (e_tgt in p.node_map):  # source and target of 'e' already mapped
                    c_src = p.node_map[e_src]
                    c_tgt = p.node_map[e_tgt]
                    if p.cod.elements_types[c_src] == 'composite_process' and p.cod.elements_types[
                        c_tgt] == 'composite_process' and p.cod.elements_labels[c_src] == p.cod.elements_labels[c_tgt]:
                        p_ext = p.extended(n_ext, {e: 'O'})
                        result = ext(p_ext, rest)
                        results += result
                        return results
                elif (e_src not in p.node_map) and (e_tgt not in p.node_map):  # unmapped source and target
                    for cproc in p.cod.nodes:
                        if p.cod.elements_types[cproc] == 'composite_process':
                            cproc_label = p.cod.elements_labels[cproc]
                            # checked no cproc node in dom that have same label so by this
                            # we avoid to map e to non decomposite cproc node
                            if not any(p.dom.elements_labels[item] == cproc_label and p.dom.elements_types[
                                item] == 'composite_process' for item in p.dom.nodes):
                                n_ext[e_src] = cproc
                                n_ext[e_tgt] = cproc
                                p_ext = p.extended(n_ext, {e: 'O'})
                                result = ext(p_ext, rest)
...
nodes_map = dict()
edges_map = dict()
abstractions_result = find_homs(dfd_c_graph, dfd_a_graph, nodes_map, edges_map)
print_homs('Concrete B-DFD', 'Abstraction B-DFD', abstractions_result)

# o = open(dfd_abstractin_filename, 'w')
# print(homs, file=o)
# o.close()

# def generate_dfd_abstraction_csv(abstractions_result, dfd_abstractin_filename):
#     dfd_abstractin = dfd_abstractin_filename
#     try:
#         with open(dfd_abstractin, 'w') as csvfile:
#             writer = csv.writer(csvfile)
#             # writer.writeheader()
#             for row in abstractions_result:
